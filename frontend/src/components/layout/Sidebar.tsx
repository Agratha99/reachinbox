...
export default function Sidebar({ scheduledCount = 0, sentCount = 0 }: SidebarProps) {
    ...
    return (
        <aside className="w-64 ios-glass border-r border-white/60 flex flex-col h-[calc(100vh-4rem)] flex-shrink-0 font-sans shadow-[6px_0_30px_rgba(0,0,0,0.03)] z-20 hidden md:flex">
            ...
            {/* Navigation List */}
            <nav className="flex-1 px-3.5 py-2 space-y-2 overflow-y-auto">
                <div className="px-3 pt-1 pb-1 text-[10px] font-black uppercase tracking-wider text-gray-400">Navigation</div>

                <Link
                    href="/dashboard/scheduled"
                    className={`flex items-center justify-between px-4 py-3 rounded-2xl text-xs font-bold transition-all ${isScheduledActive
                        ? 'bg-emerald-500/15 text-emerald-900 border border-emerald-500/30 shadow-xs'
                ...
                <Link
                    href="/dashboard/sent"
                    className={`flex items-center justify-between px-4 py-3 rounded-2xl text-xs font-bold transition-all ${isSentActive
                        ? 'bg-emerald-500/15 text-emerald-900 border border-emerald-500/30 shadow-xs'
                        : 'text-gray-600 hover:bg-white/60 hover:text-gray-900'
                        }`}
                >
